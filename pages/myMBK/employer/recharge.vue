            ...
            <div class="cash new-site">
              <el-form label-width="140px" ref="form" :model="form" :rules="rules">
                <el-form-item label="充值金额"  prop="balance">
                  <el-input class=" w-270"
                    placeholder="请输入充值金额"
                    :maxlength="30"  v-model.trim="form.balance" clearable ></el-input>
                </el-form-item>
                <el-form-item>
                  <el-button  @click="getCash" class="red-btn">充值</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
...
import { rechargeBalance } from "~/ajax/getData";
export default {
  data() {
    ...
    return {
      ...
      rules: {
        balance: [{ required: true, validator: validateAmount, trigger: "blur" }]
      }
    };
  },
  components: {
  ...
  methods: {
    getCash() {
        this.$refs.form.validate(async valid => {
            if(!valid) return ;
            let {status,returnmessage,orderNo,orderId} = await rechargeBalance(this.form)
            if(status === 1){
                location.href = '/myMBK/employer/order/detail.html?orderNo='+orderNo
            }else{
